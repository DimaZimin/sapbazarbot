...
@rate_limit(5)
@dp.callback_query_handler(text='assigned_tasks')
async def my_assigned_tasks(call: CallbackQuery):
    await call.answer(cache_time=60)
    user_id = call.from_user.id
    tasks = await db.select_assigned_tasks(user_id)
    if tasks:
        ...
        await bot.send_message(
            user_id, text="You don't assist on any assigned requests. Press /start to open main menu",
            reply_markup=None
        )
    logging.info(f'MY REQUESTS {user_id} my_assigned_tasks')
...
async def my_requests_starter(call: CallbackQuery):
    ...
    logging.info(f'MY REQUESTS {user_id} my_requests_starter')
...
async def send_list_of_requests_to_user(user_id, user_requests):
    ...
    for request in user_requests:
        ...
        assistant_records = await db.select_assistants_for_request(request_id=request_id)
        if not assistant_records:
            ...
            for assistant in assistant_records:
                assistant_contact = assistant['contact']
                assistant_name = assistant['name']
                assistant_chat = await bot.get_chat(assistant['user_id'])
                text = (f"<b>Request #{request_id}</b>\n\n"
                        f"<b>Category:</b> {request['category']}\n"
                        f"<b>Budget:</b> {request['budget']}\n")
                if request['paid']:
                    text += (f"<b>Consultant:</b> {assistant_name}\n"
                ...
                await bot.send_chat_action(user_id, 'typing')
                await asyncio.sleep(.5)
    logging.info(f'MY REQUESTS {user_id} send_list_of_requests_to_user')
...
async def send_list_of_assigned_request_to_consultant(user_id, assigned_requests):
    ...
    logging.info(f'MY REQUESTS {user_id} send_list_of_assigned_request_to_consultant')
...
async def close_request(call: CallbackQuery):
    ...
    logging.info(f'MY REQUESTS {user_id} close_request')
...
async def mark_request_as_resolved_by_consultant(call: CallbackQuery):
    ...
    logging.info(f'MY REQUESTS {user_id} mark_request_as_resolved_by_consultant')
...
async def send_notification_about_completion_of_the_request(request, user_id, consultant_id):
    ...
    logging.info(f'MY REQUESTS {user_id} send_notification_about_completion_of_the_request')
...
async def resolve_request_by_requester(call: CallbackQuery):
    ...
    logging.info(f'MY REQUESTS {user} resolve_request_by_requester')